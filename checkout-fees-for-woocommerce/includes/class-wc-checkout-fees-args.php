...
class Alg_WC_Checkout_Fees_Args {
	...
	function get_the_args_local( $current_gateway, $product_id, $variation_id, $product_qty ) {
		...
		$args['min_cart_amount']         = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_min_cart_amount_'    . $current_gateway, true ) );
		$args['max_cart_amount']         = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_max_cart_amount_'    . $current_gateway, true ) );
		$args['min_fee']                 = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_min_fee_'            . $current_gateway, true ) );
		$args['max_fee']                 = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_max_fee_'            . $current_gateway, true ) );
		$args['min_fee_2']               = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_min_fee_2_'          . $current_gateway, true ) );
		$args['max_fee_2']               = alg_wc_cf()->core->convert_currency( get_post_meta( $product_id, '_' . 'alg_checkout_fees_max_fee_2_'          . $current_gateway, true ) );
		$args['coupons_rule']            = get_post_meta( $product_id, '_' . 'alg_checkout_fees_coupons_rule_'       . $current_gateway, true );
		$args['coupons_rule_2']          = get_post_meta( $product_id, '_' . 'alg_checkout_fees_coupons_rule_2_'     . $current_gateway, true );
