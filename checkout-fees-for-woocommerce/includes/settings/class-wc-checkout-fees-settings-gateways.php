...
class Alg_WC_Checkout_Fees_Settings_Gateways {
	 ...
	 * @version 2.5.0
	 */
	function add_get_settings_hook() {
		if ( ! isset( $_GET['page'] ) || 'wc-settings' != $_GET['page'] || ! isset( $_GET['tab'] ) || 'alg_checkout_fees' != $_GET['tab'] ) {
			return;
		}
		if ( function_exists( 'WC' ) ) {
			$available_gateways = WC()->payment_gateways->payment_gateways();
	...
	function get_settings() {
		...
		if ( ! isset( $_GET['section'] ) || ! function_exists( 'WC' ) ) {
			return array();
		}
		$available_gateways = WC()->payment_gateways->payment_gateways();
		$key                = sanitize_title( $_GET['section'] );
		...
		$settings = array(
			...
			array(
				...
				'desc'      => __( 'Countries to include', 'checkout-fees-for-woocommerce' ) . apply_filters( 'alg_wc_checkout_fees_option',
					'<br>' . sprintf( __( 'You will need <a target="_blank" href="%s">Pro version</a> of the plugin to set this option.', 'checkout-fees-for-woocommerce' ),
						'https://wpfactory.com/item/payment-gateway-based-fees-and-discounts-for-woocommerce-plugin/' ), 'settings' ),
			...
			array(
				...
				'default'   => apply_filters( 'alg_wc_checkout_fees_option', '', 'cats', array( 'type' => 'include', 'fee_num' => '', 'current_gateway' => $key ) ),
				'type'      => 'multiselect',
				'class'     => 'chosen_select',
				'options'   => $product_cats,
				'custom_attributes' => apply_filters( 'alg_wc_checkout_fees_option', array( 'disabled' => 'disabled' ), 'settings' ),
			...
			array(
				...
				'default'   => apply_filters( 'alg_wc_checkout_fees_option', '', 'cats', array( 'type' => 'exclude', 'fee_num' => '', 'current_gateway' => $key ) ),
				'type'      => 'multiselect',
				'class'     => 'chosen_select',
				'options'   => $product_cats,
				'custom_attributes' => apply_filters( 'alg_wc_checkout_fees_option', array( 'disabled' => 'disabled' ), 'settings' ),
