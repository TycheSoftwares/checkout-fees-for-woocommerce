...
class Alg_WC_Checkout_Fees_Settings_Per_Product {
	...
	function enqueue_styles_and_scripts() {
		...
		wp_enqueue_script( 'checkout-fees-admin-js', alg_wc_cf()->plugin_url() .  '/includes/js/checkout-fees-admin.js',   array(), alg_wc_cf()->version );
	}
	...
	function save_meta_box( $post_id, $post ) {
		...
		foreach ( $available_gateways as $gateway_key => $gateway ) {
			...
			foreach ( $this->get_meta_box_options() as $option ) {
				...
				$option_value = ( isset( $_POST[ $option_name ] ) ? sanitize_text_field( $_POST[ $option_name ] ) : $option['default'] );
				...
				$html .= '<li>';
					...
					$html .= '<div class="tab-content" id="tab-content-' . $gateway_key . '">';
						...
						foreach ( $this->get_meta_box_options() as $option ) {
							if ( 'title' === $option['type'] ) {
								$html .= '<tr>';
								$html .= '<th style="text-align:right;padding:10px;" colspan="2">' .
									'<span style="font-size:larger;font-weight:bold;">' . $option['title'] . '</span>' . '</th>';
								$html .= '</tr>';
								continue;
							}
