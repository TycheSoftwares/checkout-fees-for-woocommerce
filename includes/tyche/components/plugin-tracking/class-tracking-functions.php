...
class Tracking_Functions {
	...
	public static function cf_get_enabled_gateways_for_fees() {
		...
		foreach ( $active_gateways as $key => $value ) {
			$args                            = pgbf_lite()->core->args_manager->get_the_args_global( $key );
			...
			$args['shipping_include']        = get_option( 'alg_gateways_fees_shipping_methods_include_fee_1_' . $key );
			$args['shipping_exclude']        = get_option( 'alg_gateways_fees_shipping_methods_exclude_fee_1_' . $key );
			...
			$args['shipping_include_2']      = get_option( 'alg_gateways_fees_shipping_methods_include_fee_2_' . $key );
			$args['shipping_exclude_2']      = get_option( 'alg_gateways_fees_shipping_methods_exclude_fee_2_' . $key );
	...
	public static function cf_get_product_count() {
		...
		foreach ( $active_gateways as $gateway => $gateway_data ) {
			...
			$product_count[ $gateway . '_product_count' ] = $count;
		}
...
$tracking_functions = new Tracking_Functions();
